	...
	public partial class MainForm : Form
	{
		...
		public MainForm()
		{
			...
			for (int i=0; i < NUM_TOASTERS; i++)
			{
				...
				PositionAndAddFlyingObject(t);
			}
			
			for (int i=0; i < NUM_TOASTS; i++)
			{
				int iSpeed = rnd_i.Next(1,10);
				Toast t = new Toast(iSpeed);
				PositionAndAddFlyingObject(t);
			}
		}
		
		
		private void PositionAndAddFlyingObject(FlyingObject object_p)
		{
			object_p.Location = new Point(rnd_i.Next(0,this.Width + this.Height), rnd_i.Next(0,this.Height + 100));
			this.Controls.Add(object_p);
		}
